[[t/TODO]](index/tags/TODO.md)](index/tags/TODO.md)

# Implementation Plan: Speech-to-text command (ligi v)
...
| 3 | Audio capture | ALSA-based capture (Linux), open `default`/`plughw` for 16 kHz | PortAudio, system tools (ffmpeg/sox) | Linux-only target makes ALSA straightforward and dependency-light; ALSA can resample |
| 4 | Model selection | Default to `base.en`, configurable via `--model-size` (`tiny|base|small|medium|large` + `.en`) or explicit `--model` path | Require explicit model path | Provides good default while allowing size/quality tradeoffs; map `large` to upstream’s current large variant (currently `large-v3`) |
...
| 8 | Time limit | Default `--timeout 10m` applies to recording only; accepts `Xs`, `Xm`, `Xh` | Unlimited | Prevents runaway recordings and bounds inference time |
...
| 11 | Model source/versioning | Pin to a vendored whisper.cpp commit and use its model list/checksums | Floating latest | Keeps URLs/checksums stable and reproducible |
...
- **Timeout semantics**: `--timeout` limits recording duration only; inference runs to completion afterward.
- **Platform**: Linux-only; command is unavailable unless built with `-Dvoice=true` on Linux.

### Options

- `--timeout <duration>`: Default `10m`. Accepts `Ns`, `Nm`, `Nh`.
- `--model-size <size>`: `tiny`, `base`, `small`, `medium`, `large`, `tiny.en`, `base.en`, `small.en`, `medium.en`. Default `base.en`. Internally, `large` resolves to the upstream “large” variant (currently `large-v3`).
...
- **Pause/Resume**: `Space` toggles pause. If stdin is not a TTY or raw key capture fails at runtime, only `Ctrl+C` is guaranteed; `Esc` and `Space` are best-effort with a warning on startup.
...
| 1 | Unsupported | Not built with voice support or non-Linux runtime |
...
### Model Source, URLs, and Checksums (SHA-256)

Pin to a vendored whisper.cpp release tag and use its model download script as the source of truth for URLs and SHA-256 values.

Pinned upstream (latest release as of 2026-01-05):
- **Repo**: `ggml-org/whisper.cpp`
- **Tag**: `v1.8.2` (release date 2025-10-15)
- **Commit**: `4979e04`

Recommended source of URLs + checksums:
- `models/download-ggml-model.sh` at the pinned tag. This script defines the base download URL and the model SHA-256 list used by whisper.cpp.
- The repo README points to Hugging Face as the manual download host (`huggingface.co/ggerganov/whisper.cpp`), so the base URL should match that host unless upstream changes it.

Implementation approach:
- Add `scripts/voice_update_models.sh` to fetch the pinned `download-ggml-model.sh`, parse the model list + SHA-256 values, and emit a Zig table for `src/voice/models.zig`.
- Keep the pinned tag constant in the script and in `src/voice/models.zig`; update both together when bumping versions.
- Avoid `main` to keep builds reproducible; only update when consciously bumping to a newer release tag.

Suggested update flow:
```
scripts/voice_update_models.sh v1.8.2
# copy generated table from art/voice_models.generated.zig into src/voice/models.zig
```
...
```
error: voice: unsupported platform (linux-only)
error: voice: not built with voice support (rebuild with -Dvoice=true)
...
| src/voice/models.zig | Model size table, filenames, base URL, checksums |
| scripts/voice_update_models.sh | (Optional) refresh checksums from pinned model URLs |
...
**Tasks**:
- Add `-Dvoice` build option to compile voice subsystem and whisper.cpp sources on Linux only.
- Link C++ stdlib when voice is enabled, and set a consistent C++ standard (prefer C++11 to match whisper.cpp).
- Compile whisper.cpp sources explicitly (likely `whisper.cpp` + `ggml.c`/`ggml-*.c` depending on pinned version) and add include paths.
...
**Tasks**:
- Define model sizes, filenames, base URL, and SHA-256 in `models.zig` based on the pinned whisper.cpp `download-ggml-model.sh`.
- Add `scripts/voice_update_models.sh` to download the pinned upstream script and regenerate the checksum table to reduce manual errors.
- Script should fail if it cannot find expected SHA-256 entries, so stale or malformed upstream scripts do not silently produce bad tables.
...
**Tasks**:
- Capture 16 kHz mono PCM via ALSA (`S16_LE`) using `default` or `plughw` so ALSA can resample if needed.
- If `snd_pcm_set_params` fails for 16 kHz on the selected device, return a clear error (no custom resampler in v1).
- Implement pause/resume by stopping buffer writes while keeping device open.
- Use termios raw mode + nonblocking stdin + `poll` to detect `Esc` and `Space`; always honor `Ctrl+C` (SIGINT).
- If stdin is not a TTY or raw key setup fails, warn and disable `Esc`/`Space` controls.
...
- Requires ALSA device support for 16 kHz capture or ALSA plug resampling; otherwise `audio capture init failed`.
- Clipboard copy depends on `wl-copy`, `xclip`, or `xsel` being available.
